...
function removeActiveMenu(content) {
    const activeMenus = document.querySelectorAll('.dropdown-content:not(.hidden)');
...
function updateParentActive(button) {
    const parentMenus = document.querySelectorAll('.dropdown-button.show');
...
function initializeMenuClicks() {
    ...
    $('.menuItemSuper_2 a').each(function() {
        const menuItemUrl = $(this).attr('href');
        if (currentUrl.indexOf(menuItemUrl) !== -1) {
            $(this).parents('.sidebar-menu').find('.menuItemSuper_1').addClass('show active');
            $(this).parents('.dropdown-content').removeClass('hidden');
        }
    });

    $('.menuItemCad_2 a').each(function() {
        const menuItemUrl = $(this).attr('href');
        if (currentUrl.indexOf(menuItemUrl) !== -1) {
            $(this).parents('.sidebar-menu').find('.menuItemCad_1').addClass('show active');
            $(this).parents('.sidebar-menu').find('.menuItemCad').addClass('show active');
            $(this).parents('.dropdown-content').removeClass('hidden');
        }
    });

    $('.menuItemProdutos_2 a').each(function() {
        const menuItemUrl = $(this).attr('href');
        if (currentUrl.indexOf(menuItemUrl) !== -1) {
            $(this).parents('.sidebar-menu').find('.menuItemProdutos_1').addClass('show active');
            $(this).parents('.dropdown-content').removeClass('hidden');
        }
    });
}
