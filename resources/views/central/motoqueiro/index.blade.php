...
@section('content')
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        @component('components.titulo-pagina', ['nome' => "Listagem dos Motoqueiros"])
        @endcomponent
        <div class="card mt-5">
            <div class="card-body">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <div>
                        ...
                        <a href="{{ route(request()->getHost().'.motoqueiros.adicionar') }}" type="button" class="ml-2 text-white btn bg-green-500 border-green-500 hover:text-white hover:bg-green-600 hover:border-green-600 focus:text-white focus:bg-green-600 focus:border-green-600 focus:ring focus:ring-green-100 active:text-white active:bg-green-600 active:border-green-600 active:ring active:ring-green-100 dark:ring-green-400/20"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="plus" class="lucide lucide-plus inline-block size-4"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg> <span class="align-middle">Novo Motoqueiro</span></a>
                    </div>
                </div>
                <table id="tableMotoqueiros" style="width:100%" class="hover group" data-url="{{ route(request()->getHost().'.motoqueiros.listar') }}">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Nome</th>
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            configTabela();
            deletarMotoqueiroClick();
        });

        const renderDefault = (data) => data ? data : "NÃO CADASTRADO";
        const renderEndereco = (data, type, row) => row.cidade_nome ? row.cidade_nome : "NÃO CADASTRADO";
        const renderActions = (data, type, row) => {
            const editarUrl = `{{ route(request()->getHost().'.motoqueiros.editar', ['id' => ':id']) }}`.replace(':id', row.id);
            const deletarUrl = `{{ route(request()->getHost().'.motoqueiros.deletar', ['id' => ':id']) }}`.replace(':id', row.id);
            return `
        ...
        const configTabela = () => {
            const rota = $('#tableMotoqueiros').data('url');
            const tabela = $('#tableMotoqueiros').DataTable({
                ...
                columns: [
                    ...
                    { data: 'cidade_nome', name: 'cidade_nome', render: renderDefault },
            ...
            $('#tableMotoqueiros').on('processing.dt', ocultarLinhasTabela);
        };

        const deletarMotoqueiroClick = () => {
