...
@section('content')
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        @component('components.titulo-pagina', ['nome' => "Listagem dos Países"])
        @endcomponent
            ...
            <div class="card-body">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <div>
                        ...
                        <button data-url="{{ route(request()->getHost().'.paises.cadastrar') }}" data-modal-target="paisModal" type="button" id="addPais" class="mb-5 ml-2 text-white btn bg-green-500 border-green-500 hover:text-white hover:bg-green-600 hover:border-green-600 focus:text-white focus:bg-green-600 focus:border-green-600 focus:ring focus:ring-green-100 active:text-white active:bg-green-600 active:border-green-600 active:ring active:ring-green-100 dark:ring-green-400/20">
                            <i data-lucide="plus" class="inline-block size-4"></i>
                            <span class="align-middle">Novo País</span>
                        </button>
                    </div>
                </div>
                <table id="tablePaises" style="width:100%" class="hover group" data-url="{{ route(request()->getHost().'.paises.listar') }}">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Nome</th>
    ...
    {{-- Modal --}}
    <div id="paisModal" modal-center="" class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
                <h5 class="text-16" id="titlePais"></h5>
                <button data-modal-close="paisModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x" class="size-5"></i></button>
            </div>
            <form action="" method="POST" id="paisForm">
                <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                    @csrf
                    <div class="mb-4">
                        <label for="nomePaisInput" class="inline-block mb-2 text-base font-medium">Nome <span class="text-red-500">*</span></label>
                        <input type="text" name="nomePaisInput" id="nomePaisInput" value="{{ old('nomePaisInput') }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                        @error('nomePaisInput')
                    ...
                    <div class="grid items-center grid-cols-1 gap-5 xl:grid-cols-2">
                        <div class="mb-4">
                            <label for="codBacenInput" class="inline-block mb-2 text-base font-medium">Cód. Bacen <span class="text-red-500">*</span></label>
                            <input type="number" name="codBacenInput" id="codBacenInput" value="{{ old('codBacenInput') }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                            @error('codBacenInput')
                                <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-4">
                            <label for="siglaPaisInput" class="inline-block mb-2 text-base font-medium">Sigla <span class="text-red-500">*</span></label>
                            <input type="text" name="siglaPaisInput" id="siglaPaisInput" value="{{ old('siglaPaisInput') }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                            @error('siglaPaisInput')
                                <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="reset" data-modal-close="paisModal" class="text-red-500 transition-all duration-200 ease-linear bg-white border-white btn hover:text-red-600 focus:text-red-600 active:text-red-600 dark:bg-zink-500 dark:border-zink-500">Fechar</button>
...
@section('script')
    ...
    <script>
        ...
        const renderActions = (data, type, row) => {
            const editarUrl = `{{ route(request()->getHost().'.paises.atualizar', ['id' => ':id']) }}`.replace(':id', row.id);
            const deletarUrl = `{{ route(request()->getHost().'.paises.deletar', ['id' => ':id']) }}`.replace(':id', row.id);
            return `
                <div class="flex gap-2">
                    <button data-item='${JSON.stringify(row)}' data-url="${editarUrl}" data-tooltip="default" data-tooltip-content="Editar" id="editPais" class="tooltip px-2 py-1.5 text-xs text-white btn bg-yellow-500 border-yellow-500 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 focus:ring focus:ring-yellow-100 active:text-white active:bg-yellow-600 active:border-yellow-600 active:ring active:ring-yellow-100 dark:ring-yellow-400/20">
        ...
        const configTabela = () => {
            const rota = $('#tablePaises').data('url');
            const tabela = $('#tablePaises').DataTable({
                ...
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'nome', name: 'nome' },
                    { data: 'codigo_bacen', name: 'codigo_bacen' },
            ...
            $('#tablePaises').on('processing.dt', ocultarLinhasTabela);
        };
        ...
        const exibirModalClick = (modalId, title, formAction, buttonText, item = null) => {
            ...
            $('#titlePais').text(title);
            $('#paisForm').attr('action', formAction);
            $('#sendForm').text(buttonText);

            if (item) {
                $('#nomePaisInput').val(item.nome);
                $('#codBacenInput').val(item.codigo_bacen);
                $('#siglaPaisInput').val(item.sigla);
            } else {
                $('#paisForm').trigger('reset');
            }
        };

        const cadastrarPaisClick = () => {
            $('#addPais').click(function () {
                const url = $(this).data('url');
                
                exibirModalClick('paisModal', 'Cadastro de País', url, 'Cadastrar');
            });
        };

        const editarPaisClick = () => {
            $(document).on('click', '#editPais', function () {
                const url = $(this).data('url');
                const item = $(this).data('item');

                exibirModalClick('paisModal', 'Edição do registro: ' + item.nome, url, 'Salvar', item);
            });
        };

        const deletarPaisClick = () => {
