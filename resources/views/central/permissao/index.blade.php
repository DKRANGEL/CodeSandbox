...
@section('content')
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            @component('components.titulo-pagina', ['nome' => "Listagem de Permissões Especiais"])
            @endcomponent
                ...
                <div class="card-body">
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <div>
                            ...
                            <button data-url="{{ route(request()->getHost().'.permissoes.cadastrar') }}" data-modal-target="permissaoModal" type="button" id="addPermissao" class="mb-5 ml-2 text-white btn bg-green-500 border-green-500 hover:text-white hover:bg-green-600 hover:border-green-600 focus:text-white focus:bg-green-600 focus:border-green-600 focus:ring focus:ring-green-100 active:text-white active:bg-green-600 active:border-green-600 active:ring active:ring-green-100 dark:ring-green-400/20">
                                <i data-lucide="plus" class="inline-block size-4"></i>
                                <span class="align-middle">Nova Permissão</span>
                            </button>
                        </div>
                    </div>
                    <table id="tablePermissoes" style="width:100%" class="hover group" data-url="{{ route(request()->getHost().'.permissoes.listar') }}">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Descrição</th>
        ...
        {{-- Modal --}}
        <div id="permissaoModal" modal-center="" class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
            <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600">
                <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
                    <h5 class="text-16" id="titlePermissao"></h5>
                    <button data-modal-close="permissaoModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x" class="size-5"></i></button>
                </div>
                <form action="" method="POST" id="permissaoForm">
                    <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                        @csrf
                        <div class="grid items-center grid-cols-1 gap-5 xl:grid-cols-1">
                            <div class="md:col-span-2 mb-4">
                                <label for="permissaoNomeInput" class="inline-block mb-2 text-base font-medium">Descrição <span class="text-red-500">*</span></label>
                                <textarea name="descricaoPermissaoInput" id="descricaoPermissaoInput" rows="2" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">{{ old('descricaoPermissaoInput') }}</textarea>
                                @error('descricaoPermissaoInput')
                                    <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
                        <div class="flex justify-end gap-2 mt-4">
                            <button type="reset" data-modal-close="permissaoModal" class="text-red-500 transition-all duration-200 ease-linear bg-white border-white btn hover:text-red-600 focus:text-red-600 active:text-red-600 dark:bg-zink-500 dark:border-zink-500">Fechar</button>
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            configTabela();
            cadastrarPermissaoClick();
            editarPermissaoClick();
            deletarPermissaoClick();
        });
 
        const configTabela = () => {
            const rota = $('#tablePermissoes').data('url');
            const tabela = $('#tablePermissoes').DataTable({
                ...
                columns: [
                    ...
                    { data: 'descricao', name: 'descricao' },
                    {
                        ...
                        render: (data, type, row) => {
                            const editarUrl = `{{ route(request()->getHost().'.permissoes.atualizar', ['id' => ':id']) }}`.replace(':id', row.id);
                            const deletarUrl = `{{ route(request()->getHost().'.permissoes.deletar', ['id' => ':id']) }}`.replace(':id', row.id);
                            return `
                                <div class="flex gap-2">
                                    <button data-item='${JSON.stringify(row)}' data-url="${editarUrl}" data-tooltip="default" data-tooltip-content="Editar" id="editPermissao" class="tooltip px-2 py-1.5 text-xs text-white btn bg-yellow-500 border-yellow-500 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 focus:ring focus:ring-yellow-100 active:text-white active:bg-yellow-600 active:border-yellow-600 active:ring active:ring-yellow-100 dark:ring-yellow-400/20">
            ...
            $('#tablePermissoes').on('processing.dt', ocultarLinhasTabela);
        };

        const cadastrarPermissaoClick = () => {
            $('#addPermissao').click(function() {
                const url = $(this).data('url');
                
                $('#permissaoModal').removeClass('hidden show');
                $('#backDropDiv').removeClass('hidden');

                $('#permissaoForm').attr('action', url);
                $('#titlePermissao').text('Cadastro de Permissão');
                $('#sendForm').text('Cadastrar');
            });
        };

        const editarPermissaoClick = () => {
            $(document).on('click', '#editPermissao', function () {
                const url = $(this).data('url');
                ...
                $('#permissaoForm').attr('action', url);
                $('#titlePermissao').text(`Edição do registro: ${item.descricao}`);
                $('#descricaoPermissaoInput').val(item.descricao);
                $('#sendForm').text('Salvar');
            });
        };

        const deletarPermissaoClick = () => {
