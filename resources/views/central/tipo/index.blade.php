...
@section('content')
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        @component('components.titulo-pagina', ['nome' => "Listagem de Tipos"])
        @endcomponent
            ...
            <div class="card-body">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <div>
                        ...
                        <button data-url="{{ route(request()->getHost().'.tipos.cadastrar') }}" data-modal-target="tipoModal" type="button" id="addTipo" class="mb-5 ml-2 text-white btn bg-green-500 border-green-500 hover:text-white hover:bg-green-600 hover:border-green-600 focus:text-white focus:bg-green-600 focus:border-green-600 focus:ring focus:ring-green-100 active:text-white active:bg-green-600 active:border-green-600 active:ring active:ring-green-100 dark:ring-green-400/20">
                            <i data-lucide="plus" class="inline-block size-4"></i>
                            <span class="align-middle">Novo Tipo</span>
                        </button>
                    </div>
                </div>
                <table id="tableTipos" style="width:100%" class="hover group" data-url="{{ route(request()->getHost().'.tipos.listar') }}">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Tipo</th>
    ...
    {{-- Modal --}}
    <div id="tipoModal" modal-center="" class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
                <h5 class="text-16" id="titleTipo"></h5>
                <button data-modal-close="tipoModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x" class="size-5"></i></button>
            </div>
            <form action="" method="POST" id="tipoForm">
                <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                    @csrf
                    <div class="mb-4">
                        <label for="tipoNomeInput" class="inline-block mb-2 text-base font-medium">Nome do Tipo <span class="text-red-500">*</span></label>
                        <input type="text" name="tipoNomeInput" id="tipoNomeInput" value="{{ old('tipoNomeInput') }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                        @error('tipoNomeInput')
                            <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="reset" data-modal-close="tipoModal" class="text-red-500 transition-all duration-200 ease-linear bg-white border-white btn hover:text-red-600 focus:text-red-600 active:text-red-600 dark:bg-zink-500 dark:border-zink-500">Fechar</button>
...
@section('script')
    ...
    <script>
        ...
        const configTabela = () => {
            const rota = $('#tableTipos').data('url');
            const tabela = $('#tableTipos').DataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                deferRender: true,
                ...
                columns: [
                    ...
                    { data: 'nome', name: 'nome' },
                    {
                        ...
                        render: (data, type, row) => {
                            const editarUrl = `{{ route(request()->getHost().'.tipos.atualizar', ['id' => ':id']) }}`.replace(':id', row.id);
                            const deletarUrl = `{{ route(request()->getHost().'.tipos.deletar', ['id' => ':id']) }}`.replace(':id', row.id);
                            return `
                                <div class="flex gap-2">
                                    <button data-item='${JSON.stringify(row)}' data-url="${editarUrl}" data-tooltip="default" data-tooltip-content="Editar" id="editTipo" class="tooltip px-2 py-1.5 text-xs text-white btn bg-yellow-500 border-yellow-500 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 focus:ring focus:ring-yellow-100 active:text-white active:bg-yellow-600 active:border-yellow-600 active:ring active:ring-yellow-100 dark:ring-yellow-400/20">
            ...
            $('#tableTipos').on('processing.dt', ocultarLinhasTabela);
        };

        const cadastrarTipoClick = () => {
            $('#addTipo').click(function() {
                const url = $(this).data('url');
                
                $('#tipoModal').removeClass('hidden show');
                $('#backDropDiv').removeClass('hidden');

                $('#divLojaSelectMultiple').prop('hidden', false);
                $('#tipoForm').attr('action', url);
                $('#titleTipo').text('Cadastro de Tipo');
                $('#sendForm').text('Cadastrar');
            });
        };

        const editarTipoClick = () => {
            $(document).on('click', '#editTipo', function () {
                const url = $(this).data('url');
                ...
                $('#divLojaSelectMultiple').prop('hidden', true);
                $('#tipoForm').attr('action', url);
                $('#titleTipo').text(`Edição do registro: ${item.nome}`);
                $('#tipoNomeInput').val(item.nome);
                $('#sendForm').text('Salvar');
            });
        };

        const deletarTipoClick = () => {
